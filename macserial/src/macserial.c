...
static const char *get_model_code(AppleModel model, bool print) {
  const char **array = AppleModelCode[model];
  int count = (int)( sizeof(array) / sizeof(*array[0]));
  int index = pseudo_random_between(0, count);
  const char *code = array[index];
...
static const char *get_board_code(AppleModel model, bool print) {
  const char **array = AppleBoardCode[model];
  int count = (int)( sizeof(array) / sizeof(*array[0]));
  int index = pseudo_random_between(0, count);
  const char *code = array[index];
...
static const char *get_board_id(AppleModel model) {
  const char *serial = AppleBoardData[model].serialNumber;
  return serial;
}
...
static int usage(const char *app) {
  printf(
    ...
    " --board <model>  (-b)  get board-id for model\n"
    " --oui            (-u)  generate Apple's MAC\n\n"
    "Tuning options:\n"
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
      ...
      mode = MODE_BOARD_ID;
    } else if (!strcmp(argv[i], "-u") || !strcmp(argv[i], "--oui")) {
      mode = MODE_GENERATE_OUI;
  ...
  if (mode == MODE_SYSTEM_INFO) {
  ...
  } else if (mode == MODE_BOARD_ID) {
    if (2 == argc) return usage(argv[0]);
    if (argv[2][0] >= '0' && argv[2][0] <= '9') {
      info.modelIndex = atoi(argv[2]);
    } else {
      for (int32_t j = 0; j < APPLE_MODEL_MAX; j++) {
        if (!strcmp(argv[2], ApplePlatformData[j].productName)) {
          info.modelIndex = j;
          break;
        }
      }
    ...
    if (info.modelIndex < 0 || info.modelIndex > APPLE_MODEL_MAX) {
      printf("Model id (%d) or name (%s) is out of valid range [0, %d]!\n", info.modelIndex, argv[2], APPLE_MODEL_MAX-1);
      return EXIT_FAILURE;
    }
    const char *id = get_board_id(info.modelIndex);
    printf("%s\n", id);
  } else if (mode == MODE_GENERATE_OUI) {
    CFUUIDRef uuid = CFUUIDCreate(kCFAllocatorDefault);
    CFUUIDBytes bytes = CFUUIDGetUUIDBytes(uuid);
    uint32_t rand_index = pseudo_random_between(0, 791);
    printf("%s:%02X:%02X:%02X\n", ouiHeader[rand_index], bytes.byte0, bytes.byte1, bytes.byte2);
    CFRelease(uuid);
  }
}
